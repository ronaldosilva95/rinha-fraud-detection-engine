...
import static br.com.rinha.fraud.detection.engine.app.configuration.InitializeConfiguration.DAYS;
import static br.com.rinha.fraud.detection.engine.app.configuration.InitializeConfiguration.HOURS;
import static br.com.rinha.fraud.detection.engine.app.configuration.InitializeConfiguration.TX_BUCKETS;
...
public class VectorSearchService {
  ...
  private static final short FRAUD_LABEL = 1;
  ...
  public double getScoreByNearestNeighbors(double[] queryVector) {
    ...
    int[] fraudsIndicator = new int[sizeBestIndex];
      ...
      for (var currentTx = txStart; currentTx <= txEnd; currentTx++) {
        ...
        for (int i = start; i < end; i++) {
          ...
          if (distance < worstDistance) {
            ...
            fraudsIndicator[worstIdx] = vectors[base + 14];
    ...
    int fraudCount = 0;
    for (var indicator : fraudsIndicator) {
      if (indicator == FRAUD_LABEL) fraudCount++;
    }

    return fraudCount / (double) sizeBestIndex;
  }

  public double getScoreByNearestNeighbors(short[] queryVector) {
    int sizeBestIndex = ApiConstants.SIZE_BEST_INDEX;
    long[] bestDistance = new long[sizeBestIndex];
    int[] bestIndexes = new int[sizeBestIndex];
    int[] fraudsIndicator = new int[sizeBestIndex];
    ...
    short q0 = queryVector[0];
    short q1 = queryVector[1];
    short q2 = queryVector[2];
    short q3 = queryVector[3];
    short q4 = queryVector[4];
    short q5 = queryVector[5];
    short q6 = queryVector[6];
    short q7 = queryVector[7];
    short q8 = queryVector[8];
    short q9 = queryVector[9];
    short q10 = queryVector[10];
    short q11 = queryVector[11];
    short q12 = queryVector[12];
    short q13 = queryVector[13];
    ...
    var txEnd = Math.min(TX_BUCKETS - 1, txBucket + 1);
    // Days mantém ±1 para cobrir fraudes de dias próximos (sábado/domingo)
    var dayStart = Math.max(0, dayBucket - 2);
    var dayEnd = Math.min(DAYS - 1, dayBucket + 2);

    for (var currentDay = dayStart; currentDay <= dayEnd; currentDay++) {
      for (var currentHour = hourStart; currentHour <= hourEnd; currentHour++) {
        for (var currentTx = txStart; currentTx <= txEnd; currentTx++) {
          var bucket = bucket(binaryBucket, currentHour, currentDay, currentTx);
          var start = bucketStarts[bucket];
          var end = bucketStarts[bucket + 1];
